...
3. Next, install the Web UI dependencies by running the following commands under the `<flexget github folder>/flexget/ui/v2/` folder, or run `python dev_tools.py build_webui`. Running `npm run build` will compile all the ui files.
